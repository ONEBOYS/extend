...
(function(win) {
	...
	win.setcookie = function(cookieName, cookieValue, config) {
		...
		document.cookie = escape(cookieName) + '=' + escape(cookieValue) + (';expires=' + opt.expires) + (';path=' + opt.path) + (';domain=' + opt.domain) + opt.secure;
	};
 ...
 * Flash包装器
 */
function FlashPlayer(wrap, url, config) {
	var config = config || {},
		id = (config.id) ? config.id : "swf" + random(),
	...
	if (config.returnType == "string") return e;
	wrapBox.insertAdjacentHTML('afterBegin', e);
	return document[id];
}

function now(f, t) {
	var D = t ? new Date(t * 1000) : new Date();
	var y = D.getFullYear();
	var m = D.getMonth() + 1;
	var w = D.getDay();
	m = m < 10 ? "0" + m : m;
